...
public class AppInfoUtil {
    ...
    public static String getFileName(HttpURLConnection connection) {
        try {
            ...
            if (connection == null || TextUtils.isEmpty(headerField)) {
                return UpdateConfig.DEFAULT_APP_SAVENAME;
            }
            return headerField.substring(headerField.lastIndexOf("filename=") + "filename=".length());
        } catch (Exception e) {
            e.printStackTrace();
            return UpdateConfig.DEFAULT_APP_SAVENAME;
        }
